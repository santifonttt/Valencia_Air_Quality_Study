---
title: "Preparación base de datos de contaminantes"
...
```{r}
df <- read.csv("~/Documents/Drive/UPV/Segundo/2ºCuatri/Proyecto/estaciones_merged/filtrados/final/datos_final.csv", stringsAsFactors = FALSE)
```
...
mediciones <- c(
  ...
  "PM2.5", "NO", "NO2", "PM10", "NOx", "SO2", "CO", "O3"
)
...
Torrent-El Vedat:
- Esta estación media 11 variables en 2019, pero de 2020 a 2024 mide 14 variables. Lo cual nos indica que se añadieron sensores.
...
Se puede observar que en 2019 no se media `CO`, `Presion` y `Radiacion_solar`. En el resto de años, 2021-2024, se miden todas, midiendo un total de 17 variables en esos ultimos años.
...
```{r}
#write.csv(df1, file = "~/Documents/Drive/UPV/Segundo/2ºCuatri/Proyecto/base_para_obj1.csv", row.names = FALSE)
```