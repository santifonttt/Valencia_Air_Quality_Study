...
fit <- aov(log_PM10 ~ Origen, data = df_pm10)
summary(fit)
```
...
Como el p-value es muy cercano a 0.05, hay una señal debil de que pudiera existir algo de desigualdad de varianzas. Pero como es mayor que alpha, no podemos rechazar la igualdad de varianzas y por ello cumple la condición de homogeniedad del ANOVA.
...
Esto indica que en estas zonas los valores de PM10 se ven afectados por su ubicación, esto podría deberse a muchos factores. Por ejemplo, a la densidad de tráfico en esas zonas, lo cual estudiaremos más adelante.
...
Con esto podemos conclui que, los valores de PM2.5 si que varian dependiendo de su estación de medicion.
...
Podemos observar que muchas de las que no tienen diferencias, estan relativamente cerca y por eso pueden tener comportamientos similares.
...
```
Las estaciones con **** indican que difieren significativamente, las demás no tienen diferencias significativas. Esas estaciones son las siguientes:
...
Estas estaciones estan todas relativamente cercas unas de otras, y por ejemplo todas sufrieron mucho durante la dana, lo que puede llevar a la conclusión de que se comportan similarmente.
